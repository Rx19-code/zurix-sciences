...
- [x] **Lot (batch) verification** — for products verified by shared lot number instead of unique per-vial code (e.g. GHK-Cu + AHK-Cu Hair Growth Spray). New collection `product_lots`. Admin endpoints: `GET/POST /api/admin/lot-batches`, `DELETE /api/admin/lot-batches/{id}` (x-admin-password). New Admin tab "🧴 Lot Verify" (`LotVerifyTab.js`): select product, enter lot number + mfg/expiry/purity, register, list, delete. `verify-product` checks `product_lots` (normalized, active) after unique_codes + product.verification_code fallbacks → always authentic, warning_level none, verification_count 0, `lot_verified=true`, NO count shown to user. Logs to verification_logs (verification_type="lot") for scan map. Manual entry only (no QR). Generic mechanism but only registered lots trigger it, so other products unaffected. NOTE: run in prod = just register the lot for the existing spray product via Admin after deploy.
- [x] **Home hero slider** (3 slides, auto-play 6s + arrows + dots, crossfade): S1 "Premium Research Compounds", S2 "Verified Authenticity" QR anti-counterfeit, S3 "Clinical Stack Protocols" Lifetime Access. testids: hero-slide-{i}, hero-dot-{i}, hero-prev/next-btn, hero-primary/secondary-btn-{i}.
- [x] **Product detail image WebP**: main image `?w=800` (~10.7KB vs 110KB), thumbnails `?w=160` + lazy loading.
...
- 2026-06: SECURITY HARDENING (app layer) — (1) get_real_ip() resolves real client IP behind Cloudflare/Nginx (cf-connecting-ip → x-forwarded-for → client.host); all 5 slowapi Limiters switched to get_real_ip (was rate-limiting globally by proxy IP). (2) Mongo-backed brute force lockout (login_attempts): 5 fails = 15min lock on user login (ip:email) and admin login (admin:ip); helpers in utils/security.py. (3) Admin Access Log: admin_access_logs collection with IP+geo+UA+success; GET /api/admin/access-logs; table in HealthTab. (4) /verify-product DoS fix: full-collection scan replaced by indexed code_normalized field (startup backfill via $replaceAll + index in server.py). (5) IPBlockMiddleware reads cf-connecting-ip first. Tested: lockout persists, other IP unaffected, login + verify regressions OK.
...
- 2026-06: Pen Dosing Guide PDF — /app/frontend/public/pen-dosing-guide.pdf (script: backend/scripts/gen_pen_dosing_pdf.py). 3ml pen = 240 clicks. Retatrutide 40mg (dose x6 clicks), Tirzepatide 60mg (dose x4 clicks), Glow Blend 70mg dosed BY GHK-Cu CONTENT per site protocols: 1mg GHK-Cu=5 clicks (beginner), 2mg=10 clicks (standard), shows per-component delivery (GHK-Cu/BPC-157/TB-500). Public URL after deploy: zurixsciences.com/pen-dosing-guide.pdf. Rendered + verified via pymupdf.
- 2026-06: A5 Product Catalog PDF (PT, commercial benefits, no prices) — script backend/scripts/gen_catalog_a5.py → frontend/public/catalogo-zurix-a5.pdf. Navy cover w/ logo, product cards (vial image + category + 3 PT benefit bullets, keyword-matched BENEFITS map covering all products incl. future pens/B12/Orforglipron), back cover with verify info + representatives. 15 pages / 47 products in preview. User must re-run script ON PRODUCTION to include newly registered products; command provided. Public URL: zurixsciences.com/catalogo-zurix-a5.pdf.
- 2026-06: Floating button CTA — white pulsing pill "Talk to a Specialist" next to green FAB (FloatingContact.js + pulse-slow keyframe in index.css). Clicking pill opens rep panel. Tested via screenshot. Analytics answered: NO GA installed; Cloudflare dashboard analytics already active (proxied domain); offered GA4 (needs Measurement ID) or CF Web Analytics (needs token) — user hasn't chosen yet.
- 2026-06: 4 NEW STACK HUBS seeded (scripts/seed_new_hubs.py, idempotent upsert by slug): tirzepatide (6 protocols incl. 60mg pen click dosing), retatrutide (6, incl. 40mg pen clicks), glow-blend (6, incl. 70mg pen 5/10 clicks by GHK-Cu), vitamin-b12 (5, incl. GLP-1 companion). Total 17 hubs. Same schema as existing (slug/peptide_slug/title/category/classification/also_known_as/description/core_info/protocols[]). Verified: API /api/hubs protocols_count OK, premium page renders (login preview@zurixsciences.com). MUST run seed on production (command given). Remaining products w/o hubs: HGH, Sermorelin, Epithalon, MOTS-c, 5-Amino-1MQ, FOXO4, SLU-PP-332, KPV, Kisspeptin, Oxytocin, AHK-Cu, PTD-DBM, Cartalax, ACE-031, Adamax, Klow Blend, NAD+, Glutathione, Orforglipron, HGH Frag.
- NOTE: frontend babel plugin (visual-edits) occasionally crashes with stale "Cannot read properties of null (reading traverse)" on Admin.js — fix: sudo supervisorctl restart frontend.
